...
#===============================================================================
etiquetas0 <- c('Falta de respeto a sus tradiciones, usos y costumbres',
                'Falta de servicios básicos (luz, agua, caminos)',
                'Que les quiten sus tierras',
                'Que el gobierno o empresas exploten sus recursos naturales sin consultarles',
                'Falta de reconocimiento a su autonomía como pueblo',
                'Falta de apoyo para conservar la lengua indigena', 
                'Persecución y violencia a quienes defienden sus territorios y recursos',
                'Otro')

#col 1
t7$TOT <- ifelse(t7$marca_indi%in%'1' , t7$FACTOR_PER, 0)
for(i in 1:8)eval(parse(text = paste0("
t7$TOT_",i," <- ifelse(t7$marca_indi%in%'1' & t7$PM1_3%in%",i,", t7$FACTOR_PER, 0)
")))
...
for(i in 1:8)eval(parse(text = paste0("
t7$TOT_",i," <- ifelse(t7$marca_indi%in%'1' & t7$PM1_3%in%",i," & t7$SEXO%in%'1', t7$FACTOR_PER, 0)
")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
...
for(i in 1:8)eval(parse(text = paste0("
t7$TOT_",i," <- ifelse(t7$marca_indi%in%'1' & t7$PM1_3%in%",i," & t7$SEXO%in%'2', t7$FACTOR_PER, 0)
")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
tab_3 <- Tab_vert_a(xx, c('Hombres',etiquetas0))

#------------------------------------------------------------------------------
tabulado <- pega_tab_1(tabla_0 = tab_1, tabla_1 = tab_3, tabla_2 = tab_2)
#-------------------------------------------------------------------------------
