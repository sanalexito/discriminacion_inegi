...
#-------------------------------------------------------------------------------
#col 1
casos<- c('En su trabajo o escuela',
          'En su familia',
          ...
          'En la policía, ministerio público o fiscalía',
          'Otro')

f0 <- eval(parse(text = paste0("t7$PM9_7_",1:9,"%in%1", collapse = "|")))

t7$TOT <- ifelse(t7$marca_indi%in%1 & f0%in%T, t7$FACTOR_PER, 0)
for(i in 1:9)eval(parse(text = paste0("t7$TOT_",i," <- ifelse(!t7$marca_indi%in%0 & t7$PM9_7_",i,"%in%'1', t7$FACTOR_PER, 0)")))
...
for(i in 1:9)eval(parse(text = paste0("t7$TOT_",i," <- ifelse(!t7$marca_indi%in%0 & t7$SEXO%in%'2' & t7$PM9_7_",i,"%in%'1', t7$FACTOR_PER, 0)")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
tab1 <- Tab_vert_a(xx, c('Estados Unidos Mexicanos', casos))

t7$TOT <- ifelse(!t7$marca_indi%in%0 & t7$SEXO%in%'1' & f0%in%T, t7$FACTOR_PER, 0)
for(i in 1:9)eval(parse(text = paste0("t7$TOT_",i," <- ifelse(!t7$marca_indi%in%0 & t7$SEXO%in%'1' & t7$PM9_7_",i,"%in%'1', t7$FACTOR_PER, 0)")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
tab2 <- Tab_vert_a(xx, c('Estados Unidos Mexicanos', casos))

#-------------------------------------------------------------------------------
tabulado0 <- pega_tab_1(tabla_0 = tab0, tabla_1 = tab1, tabla_2 = tab2)
#-------------------------------------------------------------------------------
#Parche de última hora
f0 <- eval(parse(text = paste0("t7$PM9_7_",1:9,"%in%'1'", collapse = "|")))
f1 <- eval(parse(text = paste0("t7$PM9_7_",1:9,"%in%c('2')", collapse = "&")))
