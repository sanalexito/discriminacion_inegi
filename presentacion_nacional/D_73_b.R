...
t7$TOT <- ifelse(!t7$POBF1_1%in%0 & t7$PM9_2%in%'6' &  f0%in%T , t7$FACTOR_PER, 0)
for(i in c(1:6)) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(!t7$POBF1_1%in%0 & f0%in%T & t7$PM9_3%in%",i,", t7$FACTOR_PER, 0)")))
...
#---- Mujeres ----
t7$TOT <- ifelse(!t7$POBF1_1%in%0 & t7$PM9_2%in%'6' &  f0%in%T & t7$SEXO%in%'2', t7$FACTOR_PER, 0)
for(i in c(1:6)) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(!t7$POBF1_1%in%0 & f0%in%T & t7$PM9_3%in%",i," & t7$SEXO%in%'2', t7$FACTOR_PER, 0)")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:6))
...
#---- Hombres ----
t7$TOT <- ifelse(!t7$POBF1_1%in%0 & t7$PM9_2%in%'6' &  f0%in%T & t7$SEXO%in%'1', t7$FACTOR_PER, 0)
for(i in c(1:6)) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(!t7$POBF1_1%in%0 & f0%in%T & t7$PM9_3%in%",i," & t7$SEXO%in%'1', t7$FACTOR_PER, 0)")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:6))
