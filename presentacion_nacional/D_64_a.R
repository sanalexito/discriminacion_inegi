...
t7$TOT_1 <- ifelse(!t7$afro_1%in%0 & t7$PM9_4_",i,"%in%'1', t7$FACTOR_PER, 0)
t7$TOT_2 <- ifelse(!t7$afro_1%in%0 & t7$PM9_4_",i,"%in%c('2', '3'), t7$FACTOR_PER, 0)
...
for(j in 1:2)
  for(i in 1:8)eval(parse(text = paste0("   
t7$TOT <- ifelse(  !t7$afro_1%in%0 & t7$SEXO%in%",j,", t7$FACTOR_PER, 0)
t7$TOT_1 <- ifelse(!t7$afro_1%in%0 & t7$SEXO%in%",j," & t7$PM9_4_",i,"%in%'1', t7$FACTOR_PER, 0)
t7$TOT_2 <- ifelse(!t7$afro_1%in%0 & t7$SEXO%in%",j," & t7$PM9_4_",i,"%in%c('2', '3'), t7$FACTOR_PER, 0)
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:2))
