...
etiquetas0 <- c('Sí, de acuerdo',
                'No, en desacuerdo')
#col 1
casos<- c("La mayoría de la gente cree que las personas que vienen de otro lugar generan problemas sociales a donde llegan",
          "La mayoría de la gente cree que las personas que vienen de otro lugar les quitan el empleo a las que viven ahí",
          "Las personas que vienen de otro lugar son consideradas conflictivas",
          "La mayoría de la gente considera que las personas que vienen de otro lugar rechazan las costumbres de donde llegan")

for(i in 1:4)eval(parse(text = paste0("   
t7$TOT <- ifelse(t7$migrante%in%1 , t7$FACTOR_PER, 0)
t7$TOT_1 <- ifelse(t7$migrante%in%1 & t7$PM3_3_",i,"%in%'1', t7$FACTOR_PER, 0)
t7$TOT_2 <- ifelse(t7$migrante%in%1 & t7$PM3_3_",i,"%in%'2', t7$FACTOR_PER, 0)
...
for(j in 1:2)
for(i in 1:4)eval(parse(text = paste0("   
t7$TOT <- ifelse( t7$migrante%in%1 & t7$SEXO%in%",j," , t7$FACTOR_PER, 0)
t7$TOT_1 <- ifelse(t7$migrante%in%1 & t7$SEXO%in%",j,"& t7$PM3_3_",i,"%in%'1', t7$FACTOR_PER, 0)
t7$TOT_2 <- ifelse(t7$migrante%in%1 & t7$SEXO%in%",j,"& t7$PM3_3_",i,"%in%'2', t7$FACTOR_PER, 0)
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:2))
