...
for(i in c(1:3)) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$afro_1%in%1 & f0%in%T & t7$PM9_2%in%",i,", t7$FACTOR_PER, 0)")))
t7$TOT_4 <- 1/sum(t7$TOT)
t7$TOT_5 <- 1/sum(t7$TOT)
...
for(i in c(1:3)) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$afro_1%in%1 & f0%in%T & t7$PM9_2%in%",i," & t7$SEXO%in%'2', t7$FACTOR_PER, 0)")))
t7$TOT_4 <- 1/sum(t7$TOT)
t7$TOT_5 <- 1/sum(t7$TOT)
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
...
for(i in c(1:3)) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$afro_1%in%1 & f0%in%T & t7$PM9_2%in%",i," & t7$SEXO%in%'1', t7$FACTOR_PER, 0)")))
t7$TOT_4 <- 1/sum(t7$TOT)
t7$TOT_5 <- 1/sum(t7$TOT)
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
...
t7$TOT <- ifelse(t7$afro_1%in%1 & f0%in%T , t7$FACTOR_PER, 0)
t7$TOT_1 <- ifelse(t7$afro_1%in%1 & f0%in%T & t7$PM9_2%in%c('1',2,3,5), t7$FACTOR_PER, 0)
t7$TOT_2 <- ifelse(t7$afro_1%in%1 & f0%in%T & t7$PM9_2%in%'6', t7$FACTOR_PER, 0)
...
#---- Mujeres ----
t7$TOT <- ifelse(t7$SEXO%in%'2' & t7$afro_1%in%1 & f0%in%T , t7$FACTOR_PER, 0)
t7$TOT_1<-ifelse(t7$SEXO%in%'2' & t7$afro_1%in%1 & f0%in%T & t7$PM9_2%in%c('1',2,3,5), t7$FACTOR_PER, 0)
t7$TOT_2<-ifelse(t7$SEXO%in%'2' & t7$afro_1%in%1 & f0%in%T & t7$PM9_2%in%'6', t7$FACTOR_PER, 0)
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
bla_Muj <- Tab_vert_a(xx, c("Mujeres", etiquetas))


#---- Hombres ----
t7$TOT <- ifelse(t7$SEXO%in%'1' & t7$afro_1%in%1 & f0%in%T , t7$FACTOR_PER, 0)
t7$TOT_1<-ifelse(t7$SEXO%in%'1' & t7$afro_1%in%1 & f0%in%T & t7$PM9_2%in%c('1',2,3,5), t7$FACTOR_PER, 0)
t7$TOT_2<-ifelse(t7$SEXO%in%'1' & t7$afro_1%in%1 & f0%in%T & t7$PM9_2%in%'6', t7$FACTOR_PER, 0)
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
...
#-------------------------------------------------------------------------------
  ...
  tabulado[[1]][6:7, c(2, 5, 8)] <- '0*'
  tabulado[[1]][6:7, c(3,6 ,9)] <- "0.0*"


for(i in c(2,4)) {
  tabulado[[i]][6:7, c(2, 5, 8)] <- NA
  tabulado[[i]][6:7, c(3,6 ,9)] <- NA
}

tabulado[[3]][6:7, c(2,3, 5,6,8, 9, 11:12, 14:15, 17:18)] <-"NA"
#-------------------------------------------------------------------------------
