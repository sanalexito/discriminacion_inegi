...
p1 <- eval(parse(text=paste0("t7$PM3_4_",1:6,"%in%'1' ", collapse = "|"))) 
p2 <- eval(parse(text=paste0("t7$PM3_4_",1:6,"%in%'2' ", collapse = "|"))) 
p3 <- eval(parse(text=paste0("t7$PM3_4_",1:6,"%in%'3' ", collapse = "|"))) 
p4 <- eval(parse(text=paste0("t7$PM3_4_",1:6,"%in%'4' ", collapse = "|"))) 

#bloque nacional
t7$TOT<- ifelse(!t7$migrante%in%0, t7$FACTOR_PER,0)
for(i in 1:4) eval(parse(text = paste0("
t7$TOT_",i," <- ifelse(!t7$migrante%in%0 & p",i,"%in%T,t7$FACTOR_PER,0)
 ")))
...
t7$TOT_1 <- ifelse(!t7$migrante%in%0 & p1%in%T & t7$PM3_4_",j,"%in%1, t7$FACTOR_PER, 0)
t7$TOT_2 <- ifelse(!t7$migrante%in%0 & p2%in%T & t7$PM3_4_",j,"%in%2, t7$FACTOR_PER, 0)
t7$TOT_3 <- ifelse(!t7$migrante%in%0 & p3%in%T & t7$PM3_4_",j,"%in%3, t7$FACTOR_PER, 0)
t7$TOT_4 <- ifelse(!t7$migrante%in%0 & p4%in%T & t7$PM3_4_",j,"%in%4, t7$FACTOR_PER, 0)
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
