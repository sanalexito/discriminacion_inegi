...
etiquetas0 <- c('Sí, de acuerdo',
                'No, en desacuerdo')
#col 1
casos<- c("Las personas indígenas valoran poco el seguir estudiando",
          "Las personas indígenas rechazan la tecnología (internet, computadora, celular)",
          "Las personas indígenas son poco valoradas por la mayoría de la gente",
          "Las personas indígenas son consideradas poco trabajadoras por la mayoría de la gente")

for(i in 1:4)eval(parse(text = paste0("   
t7$TOT <- ifelse( t7$marca_indi%in%'1' , t7$FACTOR_PER, 0)
t7$TOT_1 <- ifelse(t7$marca_indi%in%'1' & t7$PM1_4_",i,"%in%'1', t7$FACTOR_PER, 0)
t7$TOT_2 <- ifelse(t7$marca_indi%in%'1' & t7$PM1_4_",i,"%in%'2', t7$FACTOR_PER, 0)
...
for(j in 1:2)
for(i in 1:4)eval(parse(text = paste0("   
t7$TOT <- ifelse( t7$marca_indi%in%'1' & t7$SEXO%in%",j," , t7$FACTOR_PER, 0)
t7$TOT_1 <- ifelse(t7$marca_indi%in%'1' & t7$SEXO%in%",j," & t7$PM1_4_",i,"%in%'1', t7$FACTOR_PER, 0)
t7$TOT_2 <- ifelse(t7$marca_indi%in%'1' & t7$SEXO%in%",j," & t7$PM1_4_",i,"%in%'2', t7$FACTOR_PER, 0)
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:2))
