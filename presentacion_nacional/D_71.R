...
#-------------------------------------------------------------------------------
etiquetas0 <- c("Solo está por escrito (no puede leerlo o la letra es pequeña)",
                "Solo está en internet (carece de acceso)",
                "Se le dificulta usar el equipo (computadora, cajeros automáticos)",
                "Se le dificulta acudir a las oficinas (quedan lejos, necesita compañía, no hay transporte accesible)",
                "Le niegan la información, no le explican",
                "Desconoce dónde buscarla",
                "Se le dificulta acceder a las oficinas (carencia de rampas, señalizaciones)",
                "Oficinas cerradas o trámites parados por la pandemia",
                "La información no está disponible en formato accesible (audio, lenguaje de señas, lenguaje braille, etcétera)",
                "Otro")

#-------------------------------------------------------------------------------

f1 <- eval(parse(text = paste0("t7$PM2_5_0",1:9,"%in%'1'", collapse = "|"))) | 
                                  t7$PM2_5_10%in%'1'| t7$PM2_5_11%in%'1'
t7$TOT <- ifelse(t7$POBF1_1%in%1 & f1%in%T, t7$FACTOR_PER, 0) 
f2 <- eval(parse(text = paste0("t7$PM2_5_0",1:9,"%in%'1'", collapse = "|"))) | t7$PM2_5_10%in%'1'
t7$TOT_1 <- ifelse(t7$POBF1_1%in%1 & f2%in%T, t7$FACTOR_PER,0)
t7$TOT_2 <- ifelse(t7$POBF1_1%in%1 & t7$PM2_5_11%in%'1', t7$FACTOR_PER,0)
...
t7$TOT <- ifelse(t7$SEXO%in%'2' & t7$POBF1_1%in%1 & f1%in%T, t7$FACTOR_PER, 0)
t7$TOT_1 <- ifelse(t7$SEXO%in%'2' & t7$POBF1_1%in%1 & f2%in%T, t7$FACTOR_PER,0)
t7$TOT_2 <- ifelse(t7$SEXO%in%'2' & t7$POBF1_1%in%1 & t7$PM2_5_11%in%'1', t7$FACTOR_PER,0)
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:2))
tab1_2 <- Tab_vert_a(xx, c("Ha buscado información", 'Ha tenido algún problema', 'No ha tenido ningún problema'))

t7$TOT <- ifelse(t7$SEXO%in%'1' & t7$POBF1_1%in%1 & f1%in%T, t7$FACTOR_PER, 0)
t7$TOT_1 <- ifelse(t7$SEXO%in%'1' & t7$POBF1_1%in%1 & f2%in%T, t7$FACTOR_PER,0)
t7$TOT_2 <- ifelse(t7$SEXO%in%'1' & t7$POBF1_1%in%1 & t7$PM2_5_11%in%'1', t7$FACTOR_PER,0)
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:2))
...
#-------------------------------------------------------------------------------
termino <- c(paste0("0", 1:9), 10)

f1 <- eval(parse(text = paste0("t7$PM2_5_", termino[1:10],"%in%'1'", collapse = "|")))
t7$TOT <- ifelse(t7$POBF1_1%in%1 & f1%in%T, t7$FACTOR_PER, 0)
for(i in 1:10) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$POBF1_1%in%1 & t7$PM2_5_",termino[i],"%in%'1', t7$FACTOR_PER, 0)")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:10))
tab2_1 <- Tab_vert_a(xx, c("Estados Unidos Mexicanos",etiquetas0))

#col 2 y3 
t7$TOT <- ifelse(t7$POBF1_1%in%1 & f1%in%T & t7$SEXO%in%'2', t7$FACTOR_PER, 0)
for(i in 1:10) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$POBF1_1%in%1 & t7$PM2_5_",termino[i],"%in%'1'& t7$SEXO%in%'2', t7$FACTOR_PER, 0)")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:10))
tab2_2 <- Tab_vert_a(xx, c("Mujeres",etiquetas0))

t7$TOT <- ifelse(t7$POBF1_1%in%1 & f1%in%T & t7$SEXO%in%'1', t7$FACTOR_PER, 0)
for(i in 1:10) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$POBF1_1%in%1 & t7$PM2_5_",termino[i],"%in%'1'& t7$SEXO%in%'1', t7$FACTOR_PER, 0)")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:10))
tab2_3 <- Tab_vert_a(xx, c("Hombres",etiquetas0))

#-------------------------------------------------------------------------------

si_y_no <- c('Ha buscado información', 'Nunca ha buscado información')
t7$TOT <- ifelse(t7$POBF1_1%in%1, t7$FACTOR_PER, 0)
t7$TOT_2 <- ifelse(t7$POBF1_1%in%1 & t7$PM2_5_12%in%1, t7$FACTOR_PER, 0)
...
t7$TOT_2 <- ifelse(t7$POBF1_1%in%1 & t7$PM2_5_12%in%1 & t7$SEXO%in%'2', t7$FACTOR_PER, 0)
t7$TOT_1 <- t7$TOT-t7$TOT_2
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:2))
...
t7$TOT_2 <- ifelse(t7$POBF1_1%in%1 & t7$PM2_5_12%in%1 & t7$SEXO%in%'1', t7$FACTOR_PER, 0)
t7$TOT_1 <- t7$TOT-t7$TOT_2
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',1:2))
