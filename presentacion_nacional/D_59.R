...
for(i in 1:4) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(12:97) & t7$PM9_1_",i,"%in%'1', t7$FACTOR_PER, 0)")))
...
t7$TOT <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(12:35) & f0%in%T, t7$FACTOR_PER, 0)
t7$TOT_5 <- ifelse(t7$afro_1%in%1 &  t7$EDAD%in%c(12:35) & t7$PM9_1_5%in%'1', t7$FACTOR_PER, 0)
xx <-c('TOT', paste0('TOT_',5:5))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
tab1_2 <- Tab_vert_a(xx, c("Estados Unidos Mexicanos",etiquetas2))

t7$TOT <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(18:97) & f0%in%T, t7$FACTOR_PER, 0)
for(i in 6:8) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(18:97) & t7$PM9_1_",i,"%in%'1', t7$FACTOR_PER, 0)")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',6:8))
tab1_3 <- Tab_vert_a(xx, c("Estados Unidos Mexicanos",etiquetas3))

#---- Mujeres ----
t7$TOT <- ifelse(t7$afro_1%in%1 & f0%in%T & t7$SEXO%in%'2', t7$FACTOR_PER, 0)
for(i in 1:4) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(12:97) & t7$PM9_1_",i,"%in%'1' & t7$SEXO%in%'2', t7$FACTOR_PER, 0)")))
xx <-c('TOT', paste0('TOT_',1:4))
tab2_1 <- Tab_vert_a(xx, c("Mujeres",etiquetas1))

t7$TOT <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(12:35) & f0%in%T & t7$SEXO%in%'2', t7$FACTOR_PER, 0)
t7$TOT_5 <- ifelse(t7$afro_1%in%1 &  t7$EDAD%in%c(12:35) & t7$PM9_1_5%in%'1' & t7$SEXO%in%'2', t7$FACTOR_PER, 0)
xx <-c('TOT', paste0('TOT_',5:5))
tab2_2 <- Tab_vert_a(xx, c("Mujeres",etiquetas2))

t7$TOT <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(18:97) & f0%in%T & t7$SEXO%in%'2', t7$FACTOR_PER, 0)
for(i in 6:8) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(18:97) & t7$PM9_1_",i,"%in%'1' & t7$SEXO%in%'2', t7$FACTOR_PER, 0)")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',6:8))
tab2_3 <- Tab_vert_a(xx, c("Mujeres",etiquetas3))

#---- Hombres ----
t7$TOT <- ifelse(t7$afro_1%in%1 & f0%in%T & t7$SEXO%in%'1', t7$FACTOR_PER, 0)
for(i in 1:4) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(12:97) & t7$PM9_1_",i,"%in%'1' & t7$SEXO%in%'1', t7$FACTOR_PER, 0)")))
xx <-c('TOT', paste0('TOT_',1:4))
tab3_1 <- Tab_vert_a(xx, c("Hombres",etiquetas1))

t7$TOT <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(12:35) & f0%in%T & t7$SEXO%in%'1', t7$FACTOR_PER, 0)
t7$TOT_5 <- ifelse(t7$afro_1%in%1 &  t7$EDAD%in%c(12:35) & t7$PM9_1_5%in%'1' & t7$SEXO%in%'1', t7$FACTOR_PER, 0)
xx <-c('TOT', paste0('TOT_',5:5))
tab3_2 <- Tab_vert_a(xx, c("Hombres",etiquetas2))

t7$TOT <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(18:97) & f0%in%T & t7$SEXO%in%'1', t7$FACTOR_PER, 0)
for(i in 6:8) eval(parse(text = paste0("t7$TOT_",i," <- ifelse(t7$afro_1%in%1 & t7$EDAD%in%c(18:97) & t7$PM9_1_",i,"%in%'1' & t7$SEXO%in%'1', t7$FACTOR_PER, 0)")))
asp <- svydesign(id=~as.numeric(UPM_DIS), weights=~1, data=t7, strata=~as.numeric(EST_DIS))
xx <-c('TOT', paste0('TOT_',6:8))
